...
namespace BlockchainAssignment
{
    class Block
    {
        ...
        public DateTime timestamp; // Time when the block was created
        private int index; // Block's position in the blockchain
        public int difficulty = 4; // Number of leading zeros required in the block's hash
        ...
        // Add a difficulty parameter to the constructor
        public Block(Block lastBlock, List<Transaction> transactions, string minerAddress, int difficulty)
        {
            // Existing initialization...
            this.difficulty = difficulty; // Use the passed difficulty for mining
            hash = Mine(); // Perform the mining process with the current difficulty
        }
        ...
        // Constructor for a new block in the chain
        public Block(Block lastBlock, List<Transaction> transactions, string minerAddress, int difficulty, int halvingInterval, double initialReward)
        {
            timestamp = DateTime.Now;
            index = lastBlock.index + 1;
            prevHash = lastBlock.hash;
            // Now use the passed reward
            this.minerAddress = minerAddress; // Recipient of the mining reward
            this.difficulty = difficulty;
            Difficulty = difficulty;

            // Calculate the reward based on the halving interval
            int halvings = index / halvingInterval;
            reward = initialReward / Math.Pow(2, halvings);

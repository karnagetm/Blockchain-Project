...
namespace BlockchainAssignment
{
    class Blockchain
    {
        ...
        // Interval and reward variables
        private int halvingInterval = 210000; // The interval at which the reward is halved
        private double initialReward = 50; // The initial reward for mining a block
        ...
        public List<Transaction> GetPreferredTransactions(string preference, string minerAddress)
        {
            ...
            {
                case "Greedy":
                    // Explicitly order by fee in descending order, then by timestamp to prioritize higher fees
                ...
                case "Address Preference":
                    selectedTransactions = transactionPool
                        .Where(t => t.senderAddress == minerAddress || t.recipientAddress == minerAddress)
                        .Concat(transactionPool.Where(t => t.senderAddress != minerAddress && t.recipientAddress != minerAddress))
                        .OrderByDescending(t => t.fee) // Prioritize by fee within this preference too
        ...
        public Blockchain()
        {
            // Define the halving interval and initial reward
            int halvingInterval = 210000; // For example, Bitcoin halves every 210,000 blocks
            double initialReward = 50; // The initial reward, for example, could be 50 coins
